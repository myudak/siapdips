...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    function syncJadwalToTodoist() {
      ...
      const TODOIST_PROJECT_ID = "2360600488"; // numeric string
      const TODOIST_API_BASE = "https://api.todoist.com/rest/v2";

      // If true => only log what would happen, do NOT change Todoist
      const DRY_RUN = false;

      // =================== TYPES ===================

      interface ScrapedAssignment {
        sourceId: string; // e.g. "80337" from ?id=80337
        title: string;
        date: string; // YYYY-MM-DD
        time: string; // "23:59"
        dueIso: string; // ISO datetime string
        course: string;
        url: string;
      }
      ...
      function parseAssignmentsFromPage(): ScrapedAssignment[] {
        ...
        let currentDateTimestamp: number | null = null; // seconds

...
class SiapHandler {
  ...
  initialize() {
    ...
    // GET USER NIM
    const elementNim = document.querySelectorAll("b")[0].nextElementSibling.textContent
    console.log(elementNim);
    chrome.storage.local.set({ profileNim: elementNim });

    // GET USER PRODI
    const elementProdi = document.querySelectorAll("b")[0].nextElementSibling.nextElementSibling.nextSibling
    console.log(elementProdi);
    chrome.storage.local.set({ profileProdi: elementProdi.textContent });

    this.initialized = true;
    console.log('SIAP Handler initialized with', this.ipkElements.size, 'elements');
  ...
  collectData() {
    if (!this.initialized) this.initialize();
    return Array.from(this.ipkElements.values())
      .map(data => data.element.textContent.trim());
  }
